...
> URL 빌더 + 버튼 클릭 동작은 단위 테스트로 확인(commit 35bb87a, `lib/directions.test.ts`·`station-list.test.tsx`의 `[S6]`). Task 4와 달리 여기는 외부 SDK 설정 불확실성이 없는 순수 `window.open` 호출이라, 실 브라우저 스크린샷 없이 단위 테스트를 최저 증명 경계로 채택 — 실제 새 탭 오픈까지 포함한 전체 확인은 위치 허용이 필요해 최종 Playwright e2e로 이월(같은 샌드박스 제약, `learnings.md` 참고).
...
### Task 7: 안내/에러 상태 (결과부족 문구 + 위치거부 + API 실패) — ✅ 완료

> S8(위치 거부)은 이 샌드박스가 geolocation을 항상 거부하는 덕에 Browser MCP로 실제 화면까지 확인됨(다시 시도 클릭 → 재요청 → 콘솔 에러 없음). plan에는 없었지만 S9(다시 시도가 실제 재요청을 트리거)를 증명하려면 `hooks/use-stations.ts`에 `retry`(재시도 카운터를 effect 의존성에 포함)를 추가해야 했다 — 파일 목록에 없던 변경이라 여기 기록한다.
